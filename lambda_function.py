...
def recommend_portfolio(intent_request):
    ...
    if source =="DialogCodeHook":
        # This code performs basic validation on the supplied input splot.
        
        # Get all slots
        slots = get_slots(intent_request)

        # Validates users input using the validate_data function
        validation_result = validate_data(age, investment_amount, intent_request)

        # If the data provided by the user is not valid,
        # the ElicitSlot dialog action is used to re-prompt for the first violation detected.
        if not validation_result["isValid"]:
            slots[validation_result["violatedSlot"]] = None # Clean invaild slot
            return elicit_slot(
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            "content": """You should invest in {}. """.format(first_name, recommendation),
            
            
        },

    )
def validate_data(age,invesment_amount,intent_request):
    """ 
    Validates the data provided by the user.
    
    """
    # Validate the age, it greater than 0 and less than 65
    ...
    if age is not None:
        ...
        if age < 1 or age >= 65:
            return build_validation_result(
                False,
                "age",
                "Your age should be between 0 and 65 years old in order to use the service. Please provide a correct date of birth.",
                
            )
        # Validate the investment amount, it should be greater than or equal to 5000    
    if invesment_amount is not None: 
        dollars = parse_int(invesment_amount)
        if dollars < 5000:
            return build_validation_result(
                False,
                "investmentAmount",
                " The amount of investment has to be at least 5000. Please provide a correct amount.",
            )
    return build_validation_result(True,None,None)      

# Create a get_recommendation function to store risk_level value
